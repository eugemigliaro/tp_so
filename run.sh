...
case $OS in
    ...
    CYGWIN*|MINGW*|MSYS*)
        ...
        ACCEL_FLAGS=""
        # WHPX requiere Windows 10/11 con Hyper-V
        if qemu-system-x86_64 -accel help 2>/dev/null | grep -q "whpx"; then
            ACCEL_FLAGS="-accel whpx"
            echo "WHPX disponible - habilitando aceleración por hardware"
        elif qemu-system-x86_64 -accel help 2>/dev/null | grep -q "hax"; then
            ACCEL_FLAGS="-accel hax"
            echo "HAXM disponible - habilitando aceleración por hardware"
        else
            echo "Aceleración no disponible - ejecutando sin aceleración"
        fi
        AUDIO_CONFIG="-audiodev dsound,id=speaker -machine pcspk-audiodev=speaker $ACCEL_FLAGS"
